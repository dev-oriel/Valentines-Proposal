...
export default function Proposal() {
  ...
  // Moves the "No" button within a contained radius around its start position
  const moveButton = () => {
    setIsEscaping(true);

    // Contrain the escape radius so it doesn't leave the screen (150px)
    const radius = window.innerWidth < 768 ? 80 : 150;
    const randomX = (Math.random() - 0.5) * radius * 2;
    const randomY = (Math.random() - 0.5) * radius * 2;

    // Ensure a minimum jump distance so it's actually hard to click
    const minJump = 40;
    const finalX =
  ...
  const handleValentineYes = () => {
    // Single burst for the Valentine ask
    confetti({
      particleCount: 150,
      spread: 70,
      origin: { y: 0.6 },
      ...
      <AnimatePresence mode="wait">
        {step === 1 ? (
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-8 md:gap-12 relative">
              <button
                onClick={handleValentineYes}
                className="glow-button z-20 flex min-w-48 items-center justify-center gap-2 rounded-full h-16 px-10 bg-primary text-white text-xl font-bold transition-all cursor-pointer"
              >
              ...
              <motion.button
                ...
                onClick={moveButton}
                className="text-white bg-white/10 border border-white/20 px-10 py-4 rounded-full font-bold text-lg cursor-pointer shadow-lg z-10"
              >
                No 🫣
              </motion.button>
            </div>
          ...
          >
            <div className="space-y-6">
              <h2 className="text-cream text-xl md:text-3xl font-light italic opacity-80">
                Wait, one more thing...
              </h2>
              <h1 className="text-4xl md:text-8xl font-black text-white leading-tight">
                Will you be my <br />
              ...
              <p className="text-blush/80 text-sm md:text-lg max-w-xl mx-auto font-light leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-8 md:gap-12 relative min-h-[160px]">
              <Link to="/acceptance" onClick={handleFinalYes} className="z-20">
                <button className="glow-button flex min-w-64 items-center justify-center gap-3 rounded-full h-20 px-12 bg-primary text-white text-2xl font-black tracking-tighter transition-all hover:scale-110 cursor-pointer">
                  YES! ❤️
                </button>
              </Link>
              ...
              <motion.button
                ...
                onMouseEnter={moveButton}
                onClick={moveButton}
                className="text-white bg-white/5 border border-white/10 px-12 py-4 rounded-full font-black text-xl cursor-pointer shadow-xl z-10"
              >
                No
              </motion.button>
            </div>
